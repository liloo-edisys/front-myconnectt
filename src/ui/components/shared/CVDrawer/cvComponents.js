...
const CVDrawer = ({
  ...
  const renderPDFContent = () => {
    ...
    switch (viewerMethod) {
      case 'google':
        viewerUrl = `https://docs.google.com/gview?url=${encodeURIComponent(pdfUrl)}&embedded=true`;
        break;
      case 'pdfjs':
        viewerUrl = `https://mozilla.github.io/pdf.js/web/viewer.html?file=${encodeURIComponent(blobUrl || pdfUrl)}`;
        break;
      case 'blob':
      default:
  ...
  return (
    <>
      ...
      <div 
        ...
        className={`cv-drawer ${window.innerWidth <= 768 ? 'cv-drawer-mobile' : ''} ${className}`}
      >
        {/* Header */}
          ...
          {showControls && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <div className="btn-group btn-group-sm mr-2">
                <button 
                  className={`btn ${viewerMethod === 'google' ? 'btn-primary' : 'btn-outline-secondary'}`}
                  onClick={() => switchViewerMethod('google')}
                  title="Google Viewer"
                ...
                <button 
                  className={`btn ${viewerMethod === 'blob' ? 'btn-primary' : 'btn-outline-secondary'}`}
                  onClick={() => switchViewerMethod('blob')}
                  title="Viewer Direct"
                  style={{ fontSize: '0.75rem' }}
                >
                ...
                <button 
                  className={`btn ${viewerMethod === 'pdfjs' ? 'btn-primary' : 'btn-outline-secondary'}`}
