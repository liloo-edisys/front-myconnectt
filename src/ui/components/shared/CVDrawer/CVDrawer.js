...
const CVDrawer = ({
  ...
  const extractOriginalUrl = useCallback((googleDocsUrl) => {
    try {
      ...
      if (googleDocsUrl.includes('docs.google.com/gview?url=')) {
        ...
        const urlMatch = googleDocsUrl.match(/url=([^&]*(?:&[^&]*)*?)(?=&embedded|&SameSite|$)/);
        ...
        const simpleMatch = googleDocsUrl.match(/url=([^&]+(?:&[^&]+)*?)(?=&embedded|&SameSite)/);
  ...
  // =============================================
  const fetchPDFWithCorsHandling = useCallback(async (url) => {
    ...
    try {
      ...
      console.log('🔄 Proxies CORS publics souvent indisponibles, passage au fallback...');
    ...
    if (loading) {
      return (
        <div style={drawerStyles.loading}>
          ...
          <small className="text-muted">Tentative de résolution CORS en cours...</small>
        </div>
      );
    }
  ...
  return (
    <>
      ...
      <div 
        ...
        className={`cv-drawer ${isMobile ? 'cv-drawer-mobile' : ''} ${className}`}
      >
